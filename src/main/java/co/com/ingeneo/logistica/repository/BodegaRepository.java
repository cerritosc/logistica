package co.com.ingeneo.logistica.repository;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.util.Optional;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Slice;
import co.com.ingeneo.logistica.commons.datatables.repository.DataTablesRepository;
import co.com.ingeneo.logistica.domain.Bodega;

//@JaversSpringDataAuditable
public interface BodegaRepository extends DataTablesRepository<Bodega, Integer> {
   
	Optional<Bodega> findBySkBodega(Integer skBodega);

        Slice<Bodega> findByDescripcionIgnoreCaseContaining(String q, Pageable page);
}
